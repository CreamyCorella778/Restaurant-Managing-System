...
{
    string PhNum, Password, PasswordFromDB;
    bool endLoop = false;
    int count = 0;
    ...
    while (!endLoop)
    {
        cout << "Nhap sdt: "; cin >> PhNum;

        string sqlQuery = "select password from customer c where c.sdt = " + PhNum;

        // Execute the query
        SQLWCHAR* wszInput = new SQLWCHAR[sqlQuery.size() + 1];
        wstring sTemp = wstring(sqlQuery.begin(), sqlQuery.end());
        memcpy(wszInput, sTemp.c_str(), sqlQuery.size() + 1);
